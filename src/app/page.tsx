...
export default function HomePage() {
  ...
  // ══════════ ADMIN ══════════
  if (screen === 'admin') {
    return <AdminPanel participants={participants} onBack={() => setScreen('app')} lang={lang} setLang={setLang} />
  }
          ...
          <div className="animate-up">
            <div className="bg-white rounded-2xl border border-line p-4">
              ...
              <div className="text-primary font-bold text-lg mb-3">
                {user?.room ? `Chambre ${user.room}` : T('plan.roomConfirm')}
              </div>
              <div className="bg-warm rounded-xl p-6 text-center">
                <div className="w-12 h-12 mx-auto relative mb-3">
                  <Image src="/logo-lwm.jpeg" alt="LWM" fill className="object-contain" />
                </div>
                <div className="text-sm font-semibold text-ink">Centre des Pères Lazaristes</div>
                <div className="text-xs text-muted mt-1">Villebon-sur-Yvette, Île-de-France</div>
                <div className="mt-4 grid grid-cols-3 gap-2 text-[11px]">
                  <div className="bg-primary-l text-primary rounded-lg py-2 font-semibold">RdC<br/>100–111</div>
                  <div className="bg-sky-l text-sky rounded-lg py-2 font-semibold">1er<br/>200–212</div>
                  <div className="bg-violet-l text-violet rounded-lg py-2 font-semibold">2e<br/>300–312</div>
                </div>
              </div>
            </div>
            <div className="mt-4">
...
/* ══════════ ADMIN PANEL ══════════ */
function AdminPanel({ participants, onBack, lang, setLang }: {
  participants: Participant[]; onBack: () => void; lang: Lang; setLang: (l: Lang) => void
}) {
  const T = (key: string) => tr(key, lang)
  ...
  async function handleSend() {
    ...
    setSending(true)
    try { await ApiClient.createAnnouncement({ title, body, type, audience }); setSent(true) }
    catch { alert('Erreur') }
